"""
 CAREU AI - Coordinator Agent (Cloud Deployment - PATCHED v2)
Fixed inter-agent message handling with proper protocol registration.
...
# ============================================================================
# MESSAGE MODELS (Inline - with explicit schema registration)
# ============================================================================
...
# CRITICAL: This must match Patient Intake's IntakeTextMessage exactly
class IntakeTextMessage(Model):
    """Message to patient intake agent with symptom text"""
    text: str
    session_id: str


# CRITICAL: This must match Patient Intake's AgentAcknowledgement exactly
...
# Initialize inter-agent protocol with explicit schema registration
...
@inter_agent_proto.on_message(model=AgentAcknowledgement)
async def handle_agent_acknowledgement(ctx: Context, sender: str, msg: AgentAcknowledgement):
    """
    Handle acknowledgements from Patient Intake agent
    This includes clarification questions and confirmations
    """
    ...
    # Forward the acknowledgement message to user
...
async def startup(ctx: Context):
    ...
    ctx.logger.info("=" * 60)
    ctx.logger.info("MediChain AI Coordinator Agent (Cloud - PATCHED v2)")
    ctx.logger.info("=" * 60)
    ctx.logger.info(f"Agent address: {agent.address}")
    ctx.logger.info(f"Mailbox: Enabled (ASI:One compatible)")
    ctx.logger.info(f"Chat Protocol: Enabled")
    ctx.logger.info(f"Message handling: Protocol-based with schema registration")
    ctx.logger.info("=" * 60)
